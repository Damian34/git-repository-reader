...
import com.damian34.gitreader.infrastructure.exception.GitConnectionException;
...
public class JGitRepositoryLoader {
    ...
    private JGitRepository processLoadRepository(GitConnectionCredentials decryptedCredentials) {
        try {
            Path tempDir = JgitFileManager.createTempDirectory(tempRootFolder, UUID.randomUUID().toString());
            ...
            throw new GitConnectionException("Failed to connect to Git repository: " + e.getMessage(), e);
